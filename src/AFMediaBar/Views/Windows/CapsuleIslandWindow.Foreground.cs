...
/// makes, and automatic mode re-decides it from the island's real background.
///
...
public partial class CapsuleIslandWindow
{
    ...
    private void ApplyPlayerForeground()
    {
        ...
        // NOTE: 初版实现，待测试验证
        // 上游 #58 移除了对比阴影：DropShadowEffect 会把 TextBlock 渲染到离屏位图表面，关闭 WPF 的 ClearType 次像素渲染，
        // 文字边缘会发虚。岛内文字沿用同一裁定，不设置任何 Effect，以保证与任务栏模式行为一致。
        // Upstream #58 removed the contrast shadow: a DropShadowEffect forces the TextBlock onto an offscreen bitmap surface and
        // disables WPF's ClearType sub-pixel rendering, blurring glyph edges. Island text follows the same ruling and sets no
        // effect, so it stays consistent with taskbar mode.
    }

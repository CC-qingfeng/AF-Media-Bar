...
| Live lyrics | Lyrics exist only in taskbar rest and replace title plus artist/source when available; the Lyrics page configures the syllable reveal, unsung-part shading, credit-line filtering, the second line, and alignment; the second line takes the first source with content along the order arranged on that page (translation, romanization, next line by default), reorderable row by row, so a missing source no longer leaves a blank row; lyrics are found in order across NetEase Cloud Music, LRCLIB, QQ Music, Kugou Music, and Soda Music (each can be turned on or off and reordered there, or all turned off to stop lyric lookups entirely), and word-by-word lyrics light up with playback; the full layer names the source the current lyrics came from, and a source that answers nothing but "no lyrics" shows no lyrics at all |
| Taskbar behavior | Horizontal taskbars retain the original Rest appearance; artwork keeps its own aspect ratio (wide video covers and portrait covers are no longer cropped, and the bar width follows), Hover directly blurs/dims the original text while controls stay crisp, Full stays outside the taskbar, and a fixed target display can be selected independently |
...
| Appearance | Global fonts, font weight, player text, light/dark theme, window material (including Mica Alt), and material concentration; the accent colour follows the system by default and can also be picked from a swatch strip or a hex value (this application only, never the Windows setting); automatic text follows the actual taskbar color or wallpaper behind transparent surfaces and adds a subtle contrasting shadow on mixed backgrounds; the taskbar body is currently fixed transparent, while the island appearance (background style, opacity, radius) is shown read-only inside its own mode section |
...
| Visualizer | WASAPI loopback spectrum with four styles (bars, waveform, pixel bars, mirrored bars) that share the media text's automatic foreground and therefore follow the real background (system colours under high contrast), 9–24 bars with a fixed bar width so more bars means a wider spectrum, a 14–34 DIP spectrum height (the direction perpendicular to the taskbar, which makes the bars taller without widening anything), 5–30 Hz refresh, and adjustable jump amount; show it under Media &amp; Notifications → Rest components |

...
| 窗口模式 | 设置页展示任务栏、灵动岛、桌面卡片和悬浮球四种模式；只有任务栏是当前运行模式，后三种标记为未实现，点击只切换设置页高亮 |
...
| 托盘音频控制 | 左键单击可绑定音频面板、设置或右键菜单；普通/组合滚轮分别绑定输出设备或当前媒体音量，并与程序内滚轮共用 Shift、鼠标左键或右键修饰键 |
...
| 音频可视化 | 基于 WASAPI 回环采样的频谱在媒体连接期间保留固定槽位，播放时刷新、暂停时清零；可在“媒体与通知 → 静置层组件”中启用组件并调整 1–9 柱、5–30 Hz 与 1–400% 灵敏度 |
...
5. 右键播放器或托盘图标打开设置，在“显示模式、媒体与通知、交互、歌词、外观、应用与关于”中完成自定义。
...
| 在媒体区域向上/向下滚轮 | 普通和“修饰键 + 滚轮”可分别绑定上下首、媒体源、输出设备或当前媒体音量；默认普通滚轮切歌、Shift + 滚轮切媒体源 |
| 右键媒体栏并选择“切换媒体源” | 在多个媒体会话之间切换 |
| 在“媒体与通知 → 曲目切换通知”中启用通知 | 曲目真正切换并开始播放时显示当前曲目；可选择位置、时长、全屏策略及固定/前台目标显示器 |
...
程序启动后会延迟检查版本清单，每天最多自动检查一次。也可以在此处立即检查，并从 GitHub、夸克网盘、百度网盘或蓝奏云等已配置渠道打开下载页面。

> 说明：`src` 里目前没有任何更新检查实现（没有 `UpdateService`，也不读取 `docs/latest.json`），设置页也没有更新开关。本节描述的是发布侧渠道，不是当前程序内的功能。

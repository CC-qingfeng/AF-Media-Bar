...
| 来源交互 | 封面与标题/歌词点击结果可分别绑定为播放暂停、切回媒体应用或打开完整层；右键媒体栏切换来源；可在“媒体与通知”按应用来源启用 SMTC 允许列表 |
| 曲目切换通知 | 可选显示刚切换并开始播放的当前曲目，支持六个位置、1–10 秒、全屏抑制，以及固定显示器或前台窗口所在显示器 |
| 实时歌词 | 歌词只存在于任务栏静置层；可用时自动替换标题与歌手/来源，并在独立歌词页设置逐字擦亮、未唱部分明暗、署名行过滤、双行内容与对齐；第二行按歌词页里排定的顺序（默认 翻译 → 音译 → 下一句）取第一个有内容的来源，可逐项上移/下移，缺少所选内容时不再留空行；歌词按网易云音乐、LRCLIB、QQ 音乐、酷狗音乐与汽水音乐依次获取（可在歌词页逐个启停并调整优先级，也能全部关闭以停止联网取词），逐字歌词随播放逐字亮起；完整层显示当前歌词来自哪个来源，来源只返回「暂无歌词」时不显示歌词 |
| 任务栏适配 | 横向任务栏保留原播放器静置外观；封面按自身宽高比显示（视频类宽封面、竖版封面不再被裁切，媒体栏宽度随之变化），控制层直接模糊/淡化原文字区并保持按钮清晰，完整层位于任务栏外；可独立选择固定屏幕 |
| 窗口模式 | 设置页顶部以四张卡片展示任务栏、灵动岛、桌面卡片和悬浮球；只有任务栏是当前运行模式，后三种标记为未实现，选中只切换本页显示的区域并给出占位提示，不会切换正在运行的窗口 |
| 外观设置 | 字体、字体粗细、媒体栏文字、浅色/深色主题、窗口背景材质（含云母 Alt）与材质浓度全局统一；主题色默认跟随系统强调色，也可自选色板或十六进制值（只改本程序，不改 Windows 设置）；自动文字色按任务栏实际底色或透明区域后的壁纸切换深浅，复杂背景增加轻微反色阴影；任务栏主体当前固定透明，灵动岛外观（背景样式、不透明度、圆角）在其模式分区内只读展示 |
| 界面语言 | 简体中文、繁体中文与英文三种界面文案；默认跟随 Windows 显示语言，在「应用与关于 → 应用 → 界面语言」切换后立即生效，不需要重启程序 |
...
| 音频可视化 | 基于 WASAPI 回环采样的频谱在媒体连接期间保留固定槽位，播放时刷新、暂停时清零；颜色与媒体文字共用同一个自动前景决定，跟随实际背景一起变深变浅（高对比度下改用系统色）；可在“媒体与通知 → 静置层组件”中显示频谱并选择柱状图、波形图、像素柱状图或上下对称柱状图四种样式，调整 9–24 根柱子（柱宽固定，柱数越多频谱越宽）、14–34 DIP 频谱高度（垂直于任务栏方向，越高柱子越高）、5–30 Hz 刷新速度与 1–400% 跳动幅度 |
| 系统指标 | 横向任务栏尾部可显示紧凑性能组件，轮换系统内存、CPU、GPU 与本程序内存，采样间隔在设置里按秒调节（0.5–5 秒）；点击该组件可打开任务管理器，完整面板仍显示四项，统一协调器在无消费者时停止采样 |
...
设置页里点一下滑块就选中它（该行的边框会亮起），此时可以用左右方向键或滚轮调节：按一下走一格，按住方向键会随时间加大每步的距离；未选中的滑块不拦截滚轮，页面照常滚动。

鼠标指到媒体栏或托盘图标上会立刻显示提示：先说明普通滚轮对应哪个动作，按住共用组合键后换成组合滚轮的动作，滚动之后换成刚刚发生的结果（例如「下一首：曲名」「播放源：媒体名」）。按住鼠标键滚动时，松开按键不会触发封面/文字点击、右键菜单或托盘单击。

「应用与关于」页可以开启随 Windows 登录自动运行（默认开启），并可以「保存当前设置为默认」——之后各页的「恢复默认设置」都会回到这份自己的默认值。
...
| 操作 | 结果 |
| --- | --- |
| 点击封面 | 默认播放/暂停，也可在“交互”页改为打开播放中的程序或打开完整层 |
| 无媒体时点击或滚动音符 | 点击打开快速启动列表；滚轮循环预览，停止约 1.2 秒后启动最后一项；列表在“媒体与通知 → 快速启动”中维护 |
| 点击标题或歌词 | 默认切回当前媒体应用，也可改为播放/暂停、打开完整层或不绑定 |
| 在媒体区域向上/向下滚轮 | 普通和“按住键 + 滚轮”可分别绑定上下首、切换播放器、输出设备、当前媒体音量或不绑定；默认直接滚轮切歌、Shift + 滚轮切换播放器；滚动后的提示给出结果（切歌时曲名会在新曲目到达后更新） |
...
| 在“显示模式 → 媒体栏宽度”中关闭宽度跟随 | 在当前所需最小宽度与任务栏可用宽度之间选择固定值；放不下的标题、歌手和歌词在自己区域内轮转滚动（循环读到整句），正在逐字擦亮的歌词行则跟着亮区滚动 |
| 单击 AF Media Bar 托盘图标 | 默认打开音频控制面板；也可在“交互 → 托盘图标”中改为打开输出设备菜单、打开当前应用音量菜单、打开设置页或打开右键菜单 |
...
- 更新检查只请求两个公开的版本清单端点（`raw.githubusercontent.com` 与 `jsdelivr` 上的 `docs/latest.json`），不经任何第三方代理；安装包下载只在开启“自动下载安装”或手动点击下载时发生，直连失败时可能经清单里配置的 GH-Proxy 加速地址获取，而安装包的 SHA-256 始终来自非代理端点获取的清单。歌词功能会按当前媒体信息依次向网易云音乐、LRCLIB、QQ 音乐、酷狗音乐与汽水音乐的公开接口请求歌词（每个来源只发一次请求，命中即停止），远程封面功能会请求已配置的图片服务；这些请求都只发送曲名、歌手、专辑与时长，不上传设备信息或用户设置。
...
│       │   ├── Services/          # 业务服务
│       │   │   ├── Layout/        # 布局预设和渲染引擎
│       │   │   ├── Lyrics/        # 歌词服务（多来源取词、格式识别、逐字时间轴与当前行擦亮）
│       │   │   ├── Players/       # 媒体播放器服务
│       │   │   └── Win32/         # Windows 系统服务
│       │   ├── Settings/          # 设置管理
│       │   └── Utils/             # 工具类

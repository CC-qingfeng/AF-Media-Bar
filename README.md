...
| 实时歌词 | 歌词只存在于任务栏静置层；可用时自动替换标题与歌手/来源，并在独立歌词页设置逐字擦亮、未唱部分明暗、署名行过滤、双行内容（下一句/译文/音译）与对齐；第二行按「所选内容 → 翻译 → 音译 → 下一句」取第一个有内容的来源，缺少所选内容时不再留空行；歌词按网易云音乐、LRCLIB、QQ 音乐、酷狗音乐与汽水音乐依次获取（可在歌词页逐个启停并调整优先级，也能全部关闭以停止联网取词），逐字歌词随播放逐字亮起；完整层显示当前歌词来自哪个来源，来源只返回「暂无歌词」时不显示歌词 |
...
| 音频可视化 | 基于 WASAPI 回环采样的频谱在媒体连接期间保留固定槽位，播放时刷新、暂停时清零；颜色与媒体文字共用同一个自动前景决定，跟随实际背景一起变深变浅（高对比度下改用系统色）；可在“媒体与通知 → 静置层组件”中显示频谱并选择柱状图、波形图、像素柱状图或上下对称柱状图四种样式，调整 9–24 根柱子（柱宽固定，柱数越多频谱越宽）、5–30 Hz 刷新速度与 1–400% 跳动幅度 |

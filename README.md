...
| 来源交互 | 封面与标题/歌词点击结果可分别绑定为播放暂停或切回媒体应用；右键媒体栏切换来源；可在“额外功能”按应用来源启用 SMTC 允许列表 |
...
| 音频可视化 | 基于 WASAPI 回环采样的频谱在媒体连接期间保留固定槽位，播放时刷新、暂停时清零；可在“额外功能”调整 1–9 柱、5–30 Hz 与 1–400% 灵敏度 |
...
5. 右键播放器或托盘图标打开设置，在“显示模式、交互、歌词、全局外观、额外功能”中完成自定义。
...
| 无媒体时点击或滚动音符 | 点击打开快速启动列表；滚轮循环预览，停止约 1.2 秒后启动最后一项；列表在“额外功能”中维护 |
...
| 在“额外功能”中启用曲目切换通知 | 曲目真正切换并开始播放时显示当前曲目；可选择位置、时长、全屏策略及固定/前台目标显示器 |
...
| 点击灵动岛、桌面卡片或悬浮球模式 | 只切换设置页选中状态并显示“未实现”，不会改变任务栏运行模式 |

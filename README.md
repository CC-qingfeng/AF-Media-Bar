...
| 实时歌词 | 歌词只存在于任务栏静置层；可用时自动替换标题与歌手/来源，并在独立歌词页设置逐字擦亮、未唱部分明暗、署名行过滤、双行内容（下一句/译文/音译）与对齐；歌词按网易云音乐、LRCLIB、QQ 音乐、酷狗音乐与汽水音乐依次获取（可在歌词页逐个启停并调整优先级，也能全部关闭以停止联网取词），逐字歌词随播放逐字亮起；完整层显示当前歌词来自哪个来源，来源只返回「暂无歌词」时不显示歌词 |
